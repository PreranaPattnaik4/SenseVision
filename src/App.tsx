...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans selection:bg-blue-100 selection:text-blue-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-28 sm:h-36">
            ...
            <div className="flex items-center space-x-3.5">
              {!isAuthenticated ? (
                <span className="inline-flex items-center space-x-1.5 bg-red-50 px-3 py-1 rounded-full text-[10px] font-mono text-red-600 border border-red-200 font-bold">
                  <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse"></span>
                  <span>🔒 PERSONAL COMPANION LOCKED</span>
                </span>
              ) : (
                <span className="inline-flex items-center space-x-1.5 bg-emerald-50 px-3 py-1 rounded-full text-[10px] font-mono text-emerald-600 border border-emerald-200 font-bold">
      ...
      {/* Full screen Transparent Overlay for Voice Identity Authentication */}
      {!isAuthenticated && (
        <div className="fixed inset-0 w-full h-full bg-slate-50/75 backdrop-blur-md z-[100] overflow-y-auto flex flex-col items-center justify-start sm:justify-center p-4 sm:p-6 space-y-8 text-center animate-fade-in" id="secure-lock-screen">
          {/* Decorative Top Bar */}
          <div className="absolute top-0 inset-x-0 bg-gradient-to-r from-red-500 to-red-400 h-1.5"></div>
          
          <div className="max-w-4xl w-full mx-auto bg-white/95 p-5 sm:p-6 border border-slate-200 rounded-2xl shadow-2xl relative" id="secure-lock-content-wrapper">
